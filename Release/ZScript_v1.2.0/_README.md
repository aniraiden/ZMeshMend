...
ZBrush 网格孔洞自动修复插件。一键闭合所有开放孔洞，支持 CGAL 智能曲率感知填充、碎片移除、遮罩驱动的模型清理、边界平滑和全模型布线放松。
...
| **Smooth Open Edges** | 开放边界 Chaikin 平滑 + 多圈 falloff，保持洞口体积 |
| **Relax Wireframe** | 全模型 CGAL smooth_shape 切线方向布线放松，保持体积和细节 |
...
| `smoothIterations` | 2 | 边界平滑迭代次数 |
| `smoothRings` | 3 | 边界平滑向内扩展圈数 |
| `relaxIterations` | 3 | 全局布线放松迭代次数 |
...
A ZBrush plugin for automatic mesh hole repair. Close all open holes with one click, featuring CGAL intelligent curvature-aware filling, fragment removal, mask-driven model cleanup, boundary smoothing, and global wireframe relaxation.
...
| **Smooth Open Edges** | Chaikin boundary smoothing with multi-ring falloff, preserves hole volume |
| **Relax Wireframe** | Full-mesh CGAL smooth_shape tangent-direction wireframe relaxation |
...
| `smoothIterations` | 2 | Number of border smooth iterations |
| `smoothRings` | 3 | Number of rings to smooth inward from border |
| `relaxIterations` | 3 | Number of global wireframe relax iterations |
